...
#define ITEM_key(item) \
    (((char *)&((item)->data)) + (((item)->it_flags & ITEM_CAS) ? sizeof(uint64_t) : 0))
 ...
 */
typedef void (*ADD_STAT)(const char *key, const uint16_t klen, const char *val, const uint32_t vlen,
                         const void *cookie);
...
enum store_item_type { NOT_STORED = 0, STORED, EXISTS, NOT_FOUND, TOO_LARGE, NO_MEMORY };

enum delta_result_type { OK, NON_NUMERIC, EOM, DELTA_ITEM_NOT_FOUND, DELTA_ITEM_CAS_MISMATCH };
...
struct stats {
    ...
    uint64_t time_in_listen_disabled_us;    /* elapsed time in microseconds while server unable to
...
enum crawler_run_type { CRAWLER_AUTOEXPIRE = 0, CRAWLER_EXPIRED, CRAWLER_METADUMP };
...
static inline char *ITEM_schunk(item *it) {
    int offset = it->nkey + 1 + ((it->it_flags & ITEM_CFLAGS) ? sizeof(uint32_t) : 0) +
