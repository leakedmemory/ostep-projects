...
static void *conn_timeout_thread(void *arg) {
    ...
    if (sleep_slice == 0) sleep_slice = CONNS_PER_SLICE;
    ...
    while (do_run_conn_timeout_thread) {
        ...
        for (i = 0; i < max_fds; i++) {
            ...
            if (c->state != conn_new_cmd && c->state != conn_read) continue;

            if ((current_time - c->last_cmd_time) > settings.idle_timeout) {
                timeout_conn(c);
            } else {
                if (c->last_cmd_time < oldest_last_cmd) oldest_last_cmd = c->last_cmd_time;
            }
        }
...
static int start_conn_timeout_thread() {
    ...
    if (settings.idle_timeout == 0) return -1;
...
int stop_conn_timeout_thread(void) {
    if (!do_run_conn_timeout_thread) return -1;
    mutex_lock(&conn_timeout_lock);
...
void conn_close_idle(conn *c) {
    if (settings.idle_timeout > 0 && (current_time - c->last_cmd_time) > settings.idle_timeout) {
        ...
        if (settings.verbose > 1) fprintf(stderr, "Closing idle fd %d\n", c->sfd);
...
void thread_io_queue_add(LIBEVENT_THREAD *t, int type, void *ctx, io_queue_stack_cb cb,
                         io_queue_stack_cb com_cb, io_queue_cb ret_cb, io_queue_cb fin_cb) {
...
conn *conn_new(const int sfd, enum conn_states init_state, const int event_flags,
               ...
               struct event_base *base, void *ssl, uint64_t conntag, enum protocol bproto) {
    conn *c;
    ...
    if (init_state == conn_new_cmd) {
        LOGGER_LOG(NULL, LOG_CONNEVENTS, LOGGER_CONNECTION_NEW, NULL, &c->request_addr,
                   c->request_addr_size, c->transport, 0, sfd);
    }
...
void conn_release_items(conn *c) {
    ...
    if (c->resp_head) {
        ...
        while (resp) {
            ...
            if (resp->free) {
                fprintf(stderr,
                        "ERROR: double free detected during conn_release_items(): [%d] [%s]\n",
...
static void conn_close(conn *c) {
    ...
    if (c->thread) {
        LOGGER_LOG(c->thread->l, LOG_CONNEVENTS, LOGGER_CONNECTION_CLOSE, NULL, &c->request_addr,
                   c->request_addr_size, c->transport, c->close_reason, c->sfd);
    }
...
static const char *state_text(enum conn_states state) {
    const char *const statenames[] = {
        "conn_listening", "conn_new_cmd", "conn_waiting", "conn_read",    "conn_parse_cmd",
        "conn_write",     "conn_nread",   "conn_swallow", "conn_closing", "conn_mwrite",
        "conn_closed",    "conn_watch",   "conn_io_queue"};
...
bool resp_has_stack(conn *c) { return c->resp_head->next != NULL ? true : false; }
...
static void append_bin_stats(const char *key, const uint16_t klen, const char *val,
...
static void append_ascii_stats(const char *key, const uint16_t klen, const char *val,
                               const uint32_t vlen, conn *c) {
...
void append_stats(const char *key, const uint16_t klen, const char *val, const uint32_t vlen,
                  const void *cookie) {
...
static void complete_nread(conn *c) {
    assert(c != NULL);
#ifdef PROXY
    assert(c->protocol == ascii_prot || c->protocol == binary_prot || c->protocol == proxy_prot);
#else
...
static int _store_item_copy_chunks(item *d_it, item *s_it, const int len) {
    ...
    if (s_it->it_flags & ITEM_CHUNKED) {
        ...
        while (sch && dch && remain) {
            ...
            int todo = (dch->size - dch->used < sch->used - copied) ? dch->size - dch->used
                                                                    : sch->used - copied;
        ...
        while (len > done && dch) {
            int todo = (dch->size - dch->used < len - done) ? dch->size - dch->used : len - done;
...
static int _store_item_copy_data(int comm, item *old_it, item *new_it, item *add_it) {
    if (comm == NREAD_APPEND) {
        if (new_it->it_flags & ITEM_CHUNKED) {
            ...
            memcpy(ITEM_data(new_it) + old_it->nbytes - 2 /* CRLF */, ITEM_data(add_it),
                   add_it->nbytes);
        }
    } else {
        /* NREAD_PREPEND */
        if (new_it->it_flags & ITEM_CHUNKED) {
            ...
            memcpy(ITEM_data(new_it) + add_it->nbytes - 2 /* CRLF */, ITEM_data(old_it),
                   old_it->nbytes);
        }
    }
    return 0;
...
enum store_item_type do_store_item(item *it, int comm, conn *c, const uint32_t hv) {
    ...
    LOGGER_LOG(c->thread->l, LOG_MUTATIONS, LOGGER_ITEM_STORE, NULL, stored, comm, ITEM_key(it),
               it->nkey, it->nbytes, it->exptime, ITEM_clsid(it), c->sfd);
...
#ifndef WIN32
    append_stat("rusage_user", add_stats, c, "%ld.%06ld", (long)usage.ru_utime.tv_sec,
                (long)usage.ru_utime.tv_usec);
    append_stat("rusage_system", add_stats, c, "%ld.%06ld", (long)usage.ru_stime.tv_sec,
...
void process_stat_settings(ADD_STAT add_stats, void *c) {
    ...
    APPEND_STAT("shutdown_command", "%s", settings.shutdown_command ? "yes" : "no");
...
static void conn_to_str(const conn *c, char *addr, char *svr_addr) {
    if (!c) {
        ...
        if (c->state == conn_listening || (IS_UDP(c->transport) && c->state == conn_read)) {
            socklen_t local_addr_len = sizeof(local_addr);

            if (getsockname(c->sfd, (struct sockaddr *)&local_addr, &local_addr_len) == 0) {
...
item *limited_get(char *key, size_t nkey, conn *c, uint32_t exptime, bool should_touch,
                  bool do_update, bool *overflow) {
    item *it;
    if (should_touch) {
...
// items.c.
item *limited_get_locked(char *key, size_t nkey, conn *c, bool do_update, uint32_t *hv,
                         bool *overflow) {
    item *it;
 ...
 */
enum delta_result_type do_add_delta(conn *c, const char *key, const size_t nkey, const bool incr,
...
/*
 * multiply a delta value to a numeric item.
 *
 * c     connection requesting the operation
 * it    item to adjust
 * incr  true to mult value, false to did
 * delta amount to adjust value by
 * buf   buffer for response string
 *
 * returns a response string to send back to the client.
 */
enum delta_result_type do_mult_delta(conn *c, const char *key, const size_t nkey, const bool mult,
                                     const int64_t delta, char *buf, uint64_t *cas,
                                     const uint32_t hv, item **it_ret) {
    char *ptr;
    uint64_t value;
...
static enum try_read_result try_read_udp(conn *c) {
    ...
    c->request_addr_size = sizeof(c->request_addr);
    res = recvfrom(c->sfd, c->rbuf, c->rsize, 0, (struct sockaddr *)&c->request_addr,
                   &c->request_addr_size);
    if (res > 8) {
...
void do_accept_new_conns(const bool do_accept) {
    ...
    if (do_accept) {
        ...
        STATS_LOCK();
        elapsed_us = (maxconns_exited.tv_sec - stats.maxconns_entered.tv_sec) * 1000000 +
...
static int _transmit_pre(conn *c, struct iovec *iovs, int iovused, bool one_resp) {
    ...
    while (resp && iovused + resp->iovcnt < IOV_MAX - 1) {
        ...
        if (resp->chunked_data_iov) {
            ...
            item_chunk *ch =
                (item_chunk *)ITEM_schunk((item *)resp->iov[resp->chunked_data_iov].iov_base);
            int x;
...
static enum transmit_result transmit(conn *c) {
    ...
    if (settings.verbose > 0) perror("Failed to write, and not due to blocking");
...
static void build_udp_header(unsigned char *hdr, mc_resp *resp) {
    ...
    if (!resp->udp_total) {
        ...
        if (resp->tosend % UDP_DATA_SIZE) total++;
        ...
        switch (c->state) {
            case conn_listening:
                ...
                if (reject) {
                    ...
                    if (c->ssl_enabled) {
                        ...
                        if (ret <= 0) {
                            ...
                            if (err == SSL_ERROR_SYSCALL || err == SSL_ERROR_SSL) {
                                if (settings.verbose) {
                                    fprintf(stderr,
                                            "SSL connection failed with error code : %d : %s\n",
                                            err, strerror(errno));
                                }
                                SSL_free(ssl);
                                close(sfd);
                    ...
                    dispatch_conn_new(sfd, conn_new_cmd, EV_READ | EV_PERSIST, READ_BUFFER_CACHED,
                                      c->transport, ssl_v, c->tag, c->protocol);
                }
...
#endif /* #ifdef _WIN32 */
        if (settings.verbose > 0) perror("getsockopt(SO_SNDBUF)");
        return;
    }
...
static int server_socket(const char *interface, int port, enum network_transport transport,
                         FILE *portnumber_file, bool ssl_enabled, uint64_t conntag,
    ...
    for (next = ai; next; next = next->ai_next) {
        ...
        if (settings.sock_cookie_id != 0) {
            error = setsockopt(sfd, SOL_SOCKET, SOCK_COOKIE_ID, (void *)&settings.sock_cookie_id,
                               sizeof(uint32_t));
        ...
        if (bind(sfd, next->ai_addr, next->ai_addrlen) == -1) {
            ...
            if (portnumber_file != NULL &&
                (next->ai_addr->sa_family == AF_INET || next->ai_addr->sa_family == AF_INET6)) {
                union {
...
static int server_socket_unix(const char *path, int access_mask) {
    ...
    if (lstat(path, &tstat) == 0) {
        if (S_ISSOCK(tstat.st_mode)) unlink(path);
    }
    ...
    if (!(listen_conn = conn_new(sfd, conn_listening, EV_READ | EV_PERSIST, 1, local_transport,
                                 main_base, NULL, 0, settings.binding_protocol))) {
        fprintf(stderr, "failed to create listening connection\n");
        exit(EXIT_FAILURE);
    }
...
static const char *flag_enabled_disabled(bool flag) { return (flag ? "enabled" : "disabled"); }

static void verify_default(const char *param, bool condition) {
    if (!condition) {
        printf(
            "Default value of [%s] has changed."
...
static void usage(void) {
    ...
    printf(
        "-p, --port=<num>          TCP port to listen on (default: %d)\n"
    ...
    printf(
        ...
        (unsigned long)settings.maxbytes / (1 << 20), settings.maxconns, settings.factor,
        settings.chunk_size);
    ...
    printf(
        ...
        settings.item_size_max / (1 << 20), ITEM_SIZE_MAX_LOWER_LIMIT / (1 << 10),
    ...
    printf(
        ...
        "                          table should be. normally grows at runtime. (default starts at: "
        "%d)\n"
    ...
    printf(
        ...
        "   - warm_max_factor:     items idle > cold lru age * this drop from warm. (default: "
        "%.2f)\n"
    ...
    printf(
        "   - slab_chunk_max:      (EXPERIMENTAL) maximum slab size in kilobytes. use extreme "
        "care. (default: %d)\n"
        "   - watcher_logbuf_size: size in kilobytes of per-watcher write buffer. (default: %u)\n"
        "   - worker_logbuf_size:  size in kilobytes of per-worker-thread buffer\n"
        "                          read by background thread, then written to watchers. (default: "
        "%u)\n"
    ...
    printf(
        ...
        "   - ext_item_age:        store items idle at least this long (seconds, default: no age "
        "limit)\n"
...
static int enable_large_pages(void) {
    ...
    static const char *mm_thp_paths[] = {"/sys/kernel/mm/transparent_hugepage/enabled",
    ...
    for (const char **p = mm_thp_paths; *p; p++) {
        if ((pfd = open(*p, O_RDONLY)) != -1) break;
    }
...
static int _mc_meta_load_cb(const char *tag, void *ctx, void *data) {
    ...
    while (restart_get_kv(ctx, &key, &val) == RESTART_OK) {
        ...
        if (reuse_mmap != 0) {
            fprintf(stderr,
                    "[restart] restart incompatible due to setting for [%s] [old value: %s]\n", key,
                    val);
            break;
        }
    }
...
int main(int argc, char **argv) {
    ...
    /* process arguments */
#ifdef HAVE_GETOPT_LONG
    const struct option longopts[] = {{"unix-mask", required_argument, 0, 'a'},
        ...
        switch (c) {
            ...
            case 'I':
                ...
                if (unit == 'k' || unit == 'm' || unit == 'K' || unit == 'M') {
                    buf[strlen(buf) - 1] = '\0';
                    size_max = atoi(buf);
                    if (unit == 'k' || unit == 'K') size_max *= 1024;
                    if (unit == 'm' || unit == 'M') size_max *= 1024 * 1024;
                    ...
                    switch (getsubopt(&subopts, subopts_tokens, &subopts_value)) {
                        ...
                        case SSL_VERIFY_MODE: {
                            ...
                            switch (verify) {
                                ...
                                default:
                                    fprintf(stderr,
                                            "Invalid ssl_verify_mode. Use help to see valid "
                                            "options.\n");
                                    return 1;
                            }
                            break;
                        ...
                        case SSL_MIN_VERSION: {
                            ...
                            switch (min_version) {
                                ...
                                default:
                                    fprintf(stderr,
                                            "Invalid ssl_min_version. Use help to see valid "
                                            "options.\n");
                                    return 1;
                            }
                            break;
                        ...
                        case RESP_OBJ_MEM_LIMIT:
                            ...
                            fprintf(stderr,
                                    "DEPRECATED: resp_obj_mem_limit no longer used. See "
                                    "read_buf_mem_limit,\n");
                            break;
    ...
    if (settings.item_size_max % settings.slab_chunk_size_max != 0) {
        fprintf(stderr,
                "-I (item_size_max: %d) must be evenly divisible by slab_chunk_max (bytes: %d)\n",
    ...
    if (settings.auth_file) {
        if (!protocol_specified) {
            ...
            if (settings.binding_protocol != ascii_prot) {
                fprintf(stderr,
                        "ERROR: You cannot allow the BINARY protocol while using ascii "
    ...
    if (getrlimit(RLIMIT_NOFILE, &rlim) != 0) {
        ...
        if (setrlimit(RLIMIT_NOFILE, &rlim) != 0) {
            ...
            fprintf(stderr,
                    "failed to set rlimit for open files. Try starting as root or requesting "
...
#else
        fprintf(stderr,
                "warning: -k invalid, mlockall() not supported on this platform.  proceeding "
                "without.\n");
#endif
    }
    ...
    if (settings.auth_file) {
        ...
        switch (authfile_load(settings.auth_file)) {
            ...
            case AUTHFILE_MALFORMED:
                fprintf(stderr, "Authfile [%s] has a malformed entry. Should be 'user:password'",
                        settings.auth_file);
                exit(EXIT_FAILURE);
                break;
            case AUTHFILE_OK:
    ...
    if (settings.memory_file != NULL) {
        ...
        reuse_mem = restart_mmap_open(settings.maxbytes, settings.memory_file, &mem_base);
    ...
    if (prefill) slabs_prefill_global();
    ...
    if (settings.socketpath == NULL) {
        ...
        if (temp_portnumber_filename) free(temp_portnumber_filename);
    }
